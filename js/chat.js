...
export function renderHistorySidebar() {
    ...
    if (state.chats.length === 0) {
        historyList.innerHTML = '<p class="text-sm text-gray-500 dark:text-gray-500 px-2 italic mt-2">No recent chats</p>';
        return;
    }
    historyList.innerHTML = state.chats.map(chat => {
        ...
        return `
                <div class="relative flex items-center group w-full mb-1">
                    ...
                    <div id="dropdown-${chat.id}" class="chat-dropdown hidden absolute right-2 top-10 w-32 bg-gray-100 dark:bg-[#282a2c] border border-gray-400 dark:border-[#444749] rounded-xl shadow-xl z-50 py-1 flex flex-col">
                        <button class="rename-chat-btn w-full text-left px-4 py-2 text-sm text-gray-800 dark:text-gray-200 hover:bg-[#3f4145] hover:text-gray-900 dark:text-white transition-colors" data-id="${chat.id}">Rename</button>
...
export function appendMessageUI(msgObj, msgIndex, animate = true) {
    ...
    if (msgObj.role === 'user') {
        ...
        if (msgObj.images && msgObj.images.length > 0) {
            ...
            imageHtml = `<div class="${layoutClass} gap-2 mb-1.5 ml-auto">` + msgObj.images.map(img => `<img src="${img}" class="${imgClass} border border-gray-300 dark:border-[#333537]/80 shadow-sm" loading="lazy" decoding="async">`).join('') + `</div>`;
        }
        ...
        if (msgObj.files && msgObj.files.length > 0) {
            filesHtml = `<div class="flex flex-wrap gap-2 mb-1.5 ml-auto justify-end">` +
                msgObj.files.map(f => `
                            <div class="flex items-center gap-2 bg-gray-100 dark:bg-[#282a2c]/80 border border-gray-400 dark:border-[#444749]/50 rounded-lg px-3 py-2 shadow-sm">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-blue-400"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                                <span class="text-xs text-gray-800 dark:text-gray-200 font-mono truncate max-w-[150px]">${f.name}</span>
                            </div>
                        `).join('') + `</div>`;
        }
                    ...
                    <div class="flex-1 min-w-0 flex flex-col items-end pl-12 relative z-0">
                        ...
                        <div class="flex items-center gap-1.5">
                            <div class="flex items-center gap-0.5 msg-options">
                                <button class="copy-user-msg-btn w-9 h-9 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:bg-[#333537] transition-all" data-text="${escapedContent}" title="Copy message">
                                ...
                                <button class="edit-msg-btn w-9 h-9 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:text-blue-400 hover:bg-gray-200 dark:bg-[#333537] transition-all" data-index="${msgIndex}" title="Edit message">
        ...
        const dlBtnHtml = hasInlineImage ? `<button class="download-msg-btn w-8 h-8 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:bg-[#333537] transition-all" title="Download Image"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></button>` : '';

        const footerHtml = `
                    <div class="flex items-center gap-0.5 mt-3 pt-1">
                        <button class="copy-msg-btn w-8 h-8 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:bg-[#333537] transition-all" data-text="${escapedContent}" title="Copy response">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                        </button>
                        <button class="regen-btn w-8 h-8 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:bg-[#333537] transition-all" data-index="${msgIndex}" title="Regenerate response">
        ...
        if (msgObj.status === 'cancelled') {
            html = `<div class="msg-container flex items-start gap-4 ${animate ? 'opacity-0 transition-opacity duration-300' : ''}"><div class="w-8 h-8 shrink-0 mt-1 avatar-glow-wrapper"><div class="avatar-glow-inner"><svg class="text-gray-900 dark:text-white w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L14.4 9.6L22 12L14.4 14.4L12 22L9.6 14.4L2 12L9.6 9.6L12 2Z" fill="currentColor"/></svg></div></div><div class="flex-1 min-w-0"><div class="flex items-center gap-2 mb-1"><span class="text-sm text-gray-700 dark:text-gray-300 font-medium">${aiNameDisplay}</span><span class="text-xs text-gray-600">${timeStr}</span></div><div class="text-[15px] leading-relaxed text-gray-800 dark:text-gray-200 mt-2"><div class="inline-flex items-center gap-2 bg-gray-100 dark:bg-[#282a2c] border border-gray-300 dark:border-[#333537] px-3 py-2 rounded-lg text-gray-600 dark:text-gray-400"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span>Response was cancelled</span></div>${footerHtml}</div></div></div>`;
        } else if (msgObj.status === 'error') {
            html = `<div class="msg-container flex items-start gap-4 ${animate ? 'opacity-0 transition-opacity duration-300' : ''}"><div class="w-8 h-8 shrink-0 rounded-full flex items-center justify-center bg-gradient-to-tr from-red-600 to-orange-500 mt-1"><svg class="text-gray-900 dark:text-white w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg></div><div class="flex-1 min-w-0"><div class="flex items-center gap-2 mb-1"><span class="text-sm text-red-400 font-medium">System Error</span><span class="text-xs text-gray-600">${timeStr}</span></div><div class="text-[15px] leading-relaxed text-gray-800 dark:text-gray-200 mt-2 bg-red-900/20 border border-red-500/30 px-4 py-3 rounded-xl text-red-200">${safeContent}</div>${footerHtml}</div></div>`;
        } else {
            html = `<div class="msg-container flex items-start gap-4 group" id="${state.currentStreamingMsgId}"><div class="w-8 h-8 shrink-0 mt-1 avatar-glow-wrapper"><div class="avatar-glow-inner"><svg class="text-gray-900 dark:text-white w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L14.4 9.6L22 12L14.4 14.4L12 22L9.6 14.4L2 12L9.6 9.6L12 2Z" fill="currentColor"/></svg></div></div><div class="flex-1 min-w-0 pr-12 relative z-0"><div class="flex items-center gap-2 mb-1"><span class="text-sm text-gray-700 dark:text-gray-300 font-medium">${aiNameDisplay}</span><span class="text-xs text-gray-600">${timeStr}</span></div><div class="markdown-body text-[15px] leading-relaxed text-gray-800 dark:text-gray-200">${parseAIContent(safeContent)}</div>${footerHtml}</div></div>`;
        }
    }
...
export function showTyping() {
    ...
    const typingHtml = `<div class="msg-container flex items-start gap-4" id="${state.currentTypingId}"><div class="w-8 h-8 shrink-0 mt-1 avatar-glow-wrapper"><div class="avatar-glow-inner"><svg class="text-gray-900 dark:text-white w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L14.4 9.6L22 12L14.4 14.4L12 22L9.6 14.4L2 12L9.6 9.6L12 2Z" fill="currentColor"/></svg></div></div><div><div class="flex items-center gap-2 mb-1"><span class="text-sm text-gray-700 dark:text-gray-300 font-medium">${aiNameDisplay}</span></div><div class="flex items-center gap-1.5 h-6"><div class="w-1.5 h-1.5 bg-[#444749] rounded-full typing-dot"></div><div class="w-1.5 h-1.5 bg-[#444749] rounded-full typing-dot"></div><div class="w-1.5 h-1.5 bg-[#444749] rounded-full typing-dot"></div></div></div></div>`;
...
export async function startMessageFlow(regenText = null, regenImages = null, regenFiles = null) {
    ...
    const footerHtml = `<div class="flex items-center gap-2 mt-3 pt-2 text-gray-500 dark:text-gray-500"><button class="copy-msg-btn flex items-center gap-1.5 p-1.5 hover:bg-gray-200 dark:bg-[#333537] rounded-md text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 transition-colors" data-text="" title="Copy response" id="live-copy-${state.currentStreamingMsgId}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg></button><button class="regen-btn flex items-center gap-1.5 p-1.5 hover:bg-gray-200 dark:bg-[#333537] rounded-md text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 transition-colors" data-index="${msgIndex}" title="Regenerate response"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path><polyline points="21 3 21 8 16 8"></polyline></svg></button></div>`;
    const streamHtml = `<div class="msg-container flex items-start gap-4 group transition-opacity duration-300" id="${state.currentStreamingMsgId}"><div class="w-8 h-8 shrink-0 mt-1 avatar-glow-wrapper" id="avatar-${state.currentStreamingMsgId}"><div class="avatar-glow-inner flex justify-center items-center h-full w-full rounded-full animate-pulse-ring shadow-[0_0_15px_rgba(59,130,246,0.3)]"><svg class="text-gray-900 dark:text-white w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L14.4 9.6L22 12L14.4 14.4L12 22L9.6 14.4L2 12L9.6 9.6L12 2Z" fill="currentColor"/></svg></div></div><div class="flex-1 min-w-0 pr-12 relative z-0"><div class="flex items-center gap-2 mb-1"><span class="text-sm text-gray-700 dark:text-gray-300 font-medium">${getActivePersona().name}</span><span class="text-xs text-gray-600">${timeStr}</span></div><div class="markdown-body text-[15px] leading-relaxed text-gray-800 dark:text-gray-200" id="content-${state.currentStreamingMsgId}"><span class="text-blue-400/80 animate-pulse tracking-wide font-medium">Thinking...</span></div><div id="footer-${state.currentStreamingMsgId}" class="hidden opacity-0 transition-opacity duration-300">${footerHtml}</div></div></div>`;
...
4. IMAGE GENERATION: If the user asks you to generate, draw, or create an image, you MUST respond by creating a highly detailed prompt and outputting it as a Markdown image link. Format EXACTLY like this:
![Generated Image](${state.PYTHON_SERVER_URL}/api/generate-image?prompt=YOUR_URL_ENCODED_PROMPT_HERE)
